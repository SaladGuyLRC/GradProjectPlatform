...
interface Document { id: string; title: string; description?: string; originalFilename: string; fileSize: number; status: string; chunkCount: number; failureReason?: string; createdAt: string; updatedAt: string; indexedAt?: string }
...
async function upload() { if (!file.value) return; uploading.value = true; try { const body = new FormData(); body.append('title', form.title); body.append('description', form.description); body.append('file', file.value); await api.post('/admin/knowledge/documents', body); ElMessage.success('Document uploaded; indexing has started'); dialog.value = false; Object.assign(form, { title:'', description:'' }); file.value = null; await load() } finally { uploading.value = false } }
async function reindex(doc: Document) { await api.post(`/admin/knowledge/documents/${doc.id}/reindex`); ElMessage.success('Reindexing has started'); await load() }
async function remove(doc: Document) { await ElMessageBox.confirm(`Delete “${doc.title}” and its index?`, 'Delete document', { type:'warning' }); await api.delete(`/admin/knowledge/documents/${doc.id}`); ElMessage.success('Document deleted'); await load() }
