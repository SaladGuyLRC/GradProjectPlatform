...
interface Report { id: string; weekStart: string; weekEnd: string; completedWork: string; currentProblems?: string; nextWeekPlan: string; progressPercentage: number; status: string; studentId: string; review?: { content: string; reviewedAt: string } }
...
const form = reactive({ weekStart: '', completedWork: '', currentProblems: '', nextWeekPlan: '', progressPercentage: 0 })
onMounted(() => load())
async function load() {
  loading.value = true
  try {
...
<template>
  <div>
    <header class="page-header"><div><h1>Weekly reports</h1><p>{{ isMentor ? 'Review submissions from your students' : 'Record weekly progress and submit it for review' }}</p></div><el-button v-if="!isMentor" type="primary" :icon="Plus" @click="openCreate">Create draft</el-button></header>
    <section class="surface">
      ...
      <el-table v-loading="loading" :data="reports" empty-text="No weekly reports" @row-click="isMentor ? openDetail($event) : undefined">
        ...
        <el-table-column label="Progress" width="150"><template #default="{ row }"><el-progress :percentage="row.progressPercentage" :stroke-width="7" /></template></el-table-column>
    ...
    <el-dialog v-model="dialog" :title="editingId ? 'Edit weekly report draft' : 'Create weekly report draft'" width="650px">
      <el-form label-position="top"><el-form-item label="Week start (Monday)" required><el-date-picker v-model="form.weekStart" type="date" value-format="YYYY-MM-DD" style="width:100%" /></el-form-item><el-form-item label="Completed work" required><el-input v-model="form.completedWork" type="textarea" :rows="4" /></el-form-item><el-form-item label="Current problems"><el-input v-model="form.currentProblems" type="textarea" :rows="3" /></el-form-item><el-form-item label="Next week plan" required><el-input v-model="form.nextWeekPlan" type="textarea" :rows="3" /></el-form-item><el-form-item label="Progress"><el-slider v-model="form.progressPercentage" show-input /></el-form-item></el-form>
